...
proc/BC_IsKeyAllowedToConnect(var/key)
	...
	if(config_legacy.borderControl == BORDER_CONTROL_DISABLED)
		return 1
	else if (config_legacy.borderControl == BORDER_CONTROL_LEARNING)
...
/client/proc/BC_ToggleState()
	...
	var/choice = input("New State (Current state is: [BC_ModeToText(config_legacy.borderControl)])", "Border Control State") as null|anything in list("Disabled", "Learning", "Enforced")

	switch(choice)
		if("Disabled")
			if(config_legacy.borderControl != BORDER_CONTROL_DISABLED)
				config_legacy.borderControl = BORDER_CONTROL_DISABLED
				log_and_message_admins("[key_name(usr)] has disabled border control.")
		if("Learning")
			if(config_legacy.borderControl != BORDER_CONTROL_LEARNING)
				config_legacy.borderControl = BORDER_CONTROL_LEARNING
		...
		if("Enforced")
			if(config_legacy.borderControl != BORDER_CONTROL_ENFORCED)
				config_legacy.borderControl = BORDER_CONTROL_ENFORCED
