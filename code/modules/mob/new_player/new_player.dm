		...
		if(!client.prefs)
			...
			if(!hasverified) //they have not completed age gate
				...
				if(verify == FALSE)
					...
					to_chat(src, "You have failed the initial age verification check. \nIf you believe this was in error, you MUST submit to additional verification on the Crescent 13 Discord.")
					if(client)
						AddBan(ckey, computer_id, "Failed initial age verification check. Appeal on the Crescent 13 Discord.", "SYSTEM", 0, 0)
						Logout()
					return FALSE
				else
